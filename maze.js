...
//  Look down
    if (current < (boardSize - width) 
        && !board[south].classList.contains('checked')) {
        options.push(board[south].id)
    }
//  Look left
    if (current > 0 && current % width !== 0 && !board[west].classList.contains('checked')) {
...
//  Backtrack until options.length
    ...
    recursion(30) // <<<<<<<<<<<<<<<<<<<<<<<<<<<  Set maze build animation apeed
}

document.addEventListener('keydown', event => {
    message.style.opacity = 0
//  Up key
    if (board[current].classList.contains('top-none') && event.key === 'ArrowUp') {
        board[current].classList.remove('player')
        current -= width
        board[current].classList.add('player')
    }
//  Right key
    if (board[current].classList.contains('right-none') && event.key === 'ArrowRight') {
        board[current].classList.remove('player')
        current++
        board[current].classList.add('player')
    }
//  Down key
    if (board[current].classList.contains('bottom-none') && event.key === 'ArrowDown') {
        board[current].classList.remove('player')
        current += width
        board[current].classList.add('player')
    }
//  Left key
    if (board[current].classList.contains('left-none') && event.key === 'ArrowLeft') {
        board[current].classList.remove('player')
        current--
        board[current].classList.add('player')
    }
